...
pub const CURRENT_BLOCK_HEIGHT: u64 = 951387;
...
pub fn is_mainnet(network: &str) -> bool {
    ...
    network.to_lowercase().eq("mainnet")
}
...
pub fn generate_address(prefix: &str) -> String {
    ...
    for _ in 0..(32 - prefix.len()) {
        let numbers = rang.gen_range(0..36);

        let xters = if numbers < 26 {
            (b'a' + numbers as u8) as char
        } else {
            (b'0' + (numbers - 26) as u8) as char
        };
...
pub fn find_utxo_with_min_value(utxos: &[Utxo], target: u64) -> Option<Utxo> {
    ...
    let mut smallest: Option<&Utxo> = None;

    for utxo in utxos {
        if utxo.value >= target {
            match smallest {
                None => smallest = Some(utxo),
                Some(s) if utxo.value < s.value => smallest = Some(utxo),
                _ => {}
            }
        }
    }
